...
test('discovers monitored temperature from SafetyComponent derivative pair', () => {
  const entities: EntityMap = {
    'sensor.example_office_temperature': entity('23.25', {
      friendly_name: 'Biuro',
      unit_of_measurement: '°C',
    }),
    'sensor.example_office_temperature_rate': entity('-0.015', {
      unit_of_measurement: '°C/min',
      attribution: 'Data provided by SafetyFunction',
    }),
    'sensor.example_office_temperature_rateofrate': entity('0.001', {
      unit_of_measurement: '°C/min²',
    }),
    'sensor.example_office_temperature_low_threshold': entity('18', {
      friendly_name: 'Dolny próg temperatury — Biuro',
      source_entity: 'sensor.example_office_temperature',
      threshold_type: 'low',
      unit_of_measurement: '°C',
    }),
    'sensor.example_office_temperature_high_threshold': entity('28', {
      source_entity: 'sensor.example_office_temperature',
      threshold_type: 'high',
      unit_of_measurement: '°C',
    }),
...
test('discovers only configured Safety Doors MQTT entities', () => {
  const doors = getSafetyDoors({
    ...
    'sensor.example_occupancy': entity('occupied', {
  ...
  assert.equal(doors[1].conditionEntityId, 'sensor.example_occupancy');
