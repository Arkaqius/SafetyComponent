...
import { formatNotificationTime } from '../domain/notificationHistory.js';
import type { EntitySnapshot } from '../domain/safety.js';

const historyEntity: EntitySnapshot = {
  state: '2',
  attributes: {
    version: 1,
    limit: 100,
...
test('renders accepted notification content, recipient and diagnostic details', () => {
  const markup = renderToStaticMarkup(<NotificationHistory connected entity={historyEntity} />);
...
test('shows disconnected state while retaining already read entries', () => {
  const markup = renderToStaticMarkup(<NotificationHistory connected={false} entity={historyEntity} />);
