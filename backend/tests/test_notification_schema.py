...
def test_defaults_use_explicit_example_service_and_home_assistant_url() -> None:
    config = validate_notification_config({})

    assert config["mobile"]["services"] == ["notify/mobile_app_example_phone"]
    assert config["mobile"]["default_url"] == "/"
