...
def test_installation_component_override_replaces_system_override():
    runtime = validate_entity_monitor_config(
        {
            "explicit_entities": {},
            "component_overrides": {
                "TemperatureExampleRoom": {
                    "detection_budget_seconds": 315,
                    "checks": {
                        "freshness": {
                            "timestamp_source": "last_updated",
    ...
    override = runtime["component_overrides"]["TemperatureExampleRoom"]
    assert override["detection_budget_seconds"] == 315
    assert override["checks"]["freshness"]["max_silence_seconds"] == 300
