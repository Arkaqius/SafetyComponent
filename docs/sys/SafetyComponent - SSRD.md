...
| SWR-FSM-002 | The application shall record each enabled component's last completed evaluation time and result. Periodic components shall additionally record expected interval and missed-deadline count; event-driven components shall identify their trigger mode and must not treat an idle period alone as a missed evaluation. A heartbeat, successful callback registration, or one component's evaluation shall not make another component healthy. The aggregate view shall keep unknown and overdue components visible separately from `sensor.safety_app_health` and `sensor.safetysystem_state`. | component evaluation instrumentation and diagnostic registry |
...
| SWR-FSM-006 | Network observation shall distinguish local Home Assistant transport, MQTT transport, WAN reachability, and independent external providers; it shall not interpret one failed probe as proof of all Internet loss or treat an unknown route as online. Confirmed WAN loss shall assert a level-3 network fault. The WAN rule shall use reachability only, without packet-loss or latency classification. | network monitor and notification integration |
...
| SWR-FSM-008 | Valid `sensor` battery percentages or `binary_sensor` low-battery states for remote devices shall be associated with stable device identity and produce one level-4 maintenance condition per device below system-owned calibration or on a valid low assertion. At startup, automatic discovery shall use enabled Home Assistant entity-registry entries, device associations, and matching battery device classes; percentage sensors shall use `%` units. Device exclusions shall use Home Assistant device registry IDs, and explicit manual bindings shall not be monitored a second time by discovery. The authenticated editor shall expose the discovered inventory and monitoring selection; saving and restarting the App shall apply selections. Failed discovery shall expose unknown coverage. Unavailable or invalid readings shall not prove low or full charge; the owning component's unavailable-input fault remains independent. | device maintenance policy |
...
| SWR-FSM-011 | When SafetyFunctions is down, its in-process checks shall not be claimed as operational. A complete loss of the process, Home Assistant, or the host shall remain uncovered unless an independent observer and reporting path are installed; an AppDaemon timer or MQTT heartbeat shall not be treated as that observer. | deployment supervision boundary |
...
| SWR-FSM-013 | The backend shall expose notification attempt, Home Assistant acceptance, retry/queue state, and observable channel failure separately from confirmed recipient delivery. It shall expose recovery command and postcondition status from the owning RecoveryManager without inferring effect from command acceptance. The frontend shall show unknown effect or delivery explicitly and shall not merge these with coverage health. | NotificationManager, RecoveryManager, diagnostic registry, frontend |
| SWR-FSM-014 | Detector maintenance shall retain one test schedule and last valid result per configured smoke, gas, or carbon-monoxide detector. Device-reported tests and operator attestations shall be identified by source and time; a normal detector state shall not count as a test. The frontend shall show due/overdue status and allow an authenticated operator to record a manual test result without invoking detector control or clearing an active alarm. | maintenance state store, API, frontend |
| SWR-FSM-015 | Free-disk-space and host-temperature rules shall use installation-bound host sensors, valid compatible units, finite plausible values, source freshness, independent qualification, and recovery hysteresis. Qualified low-space or high-temperature conditions shall assert separate level-4 maintenance faults. Invalid samples shall reset pending qualification/recovery evidence without positively clearing an active fault. | platform telemetry adapter, resource policies, FaultManager |
| SWR-FSM-016 | Backup evaluation shall parse a timezone-aware last-success timestamp and an optional fresh binary problem source. A valid old success shall be overdue; a valid asserted failure shall be failed; malformed, future, unavailable, or transport-failed evidence shall not pass or clear an active condition. The backend shall expose separate backup evidence and level-4 overdue/failed maintenance without invoking backup creation or restore. | backup policy and functional safety diagnostics |
| SWR-FSM-017 | The backend shall persist authenticated `safety_periodic_test_result` events only for enabled `notification_delivery` or `backup_restore` keys and explicit `passed`/`failed` outcomes, stamping source `operator_attestation` and completion time. `sensor.safety_periodic_tests` shall publish bounded per-test status, last result/time, due time, and interval. Missing records shall be due, expired passes overdue, failures failed, and unreadable history unknown; due/overdue/failed states shall produce level-4 maintenance. The frontend shall explain actual receipt and separate-installation restore and shall submit no notification, actuator, or restore command. | periodic test monitor, durable state store, authenticated frontend |
...
| SG-003 Functional safety and platform health | `SYS-SR-FSM-001..017` | `SWR-FSM-001..017` |
