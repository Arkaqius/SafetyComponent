...
temperature, heating-system, safety-door, and external-hazard monitoring, fault aggregation,
...
| SWR-DOOR-008 | The current installation calibration shall use 300 s for `GarageGate`, 600 s for `ExternalGate`, 120 s for `LivingRoomTerraceDoor`, and 900 s for `GarageDoor`. | `backend/config/system_config.yml` and generated `backend/app_cfg.yaml` |
...
| SWR-NOT-021 | New, updated, and repeated fault notifications shall appear as `SET`; resolved notifications as `CLEARED`; and notification removal after shadowing as `SHADOWED`. Shadowing shall not be represented as fault healing. | notification history lifecycle mapping |
| SWR-NOT-022 | `sensor.notification_history` shall publish the retained count, versioned entries newest first, and retention limit at manager startup and after submission attempts. WAN queueing without a submission shall not create a history entry. | MQTT notification history |
